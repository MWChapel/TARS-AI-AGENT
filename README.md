...
| `PERSON` | unset | Personalize who TARS is talking to. A single name (`PERSON=Michael`) is always used. A comma-separated list (`PERSON=Michael,Lynda,Olivia`) picks one name at random once per session — the pick doesn't change mid-conversation. Unset means no personalization. |
