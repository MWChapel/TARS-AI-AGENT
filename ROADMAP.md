...
5. **Tool-execution log** — the Electron UI no longer has a dedicated call-log panel (replaced by the cosmetic ORACLE panel), so this phase needs a UI decision: either add a lightweight tool-execution log panel back, or surface tool calls inline in the mission log itself.
